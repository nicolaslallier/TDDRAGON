"""
Integration tests for main.py in real application scenarios.

Tests application startup, lifespan, and main function with real FastAPI app.
"""
...
class TestMainIntegration:
    """Integration test suite for main.py."""
    ...
    def test_lifespan_startup_initializes_database(self):
        ...
        with patch("src.endpoints.demo_api.main.init_database") as mock_init, patch(
            ...
            # Verify startup logging (line 65)
            startup_calls = [
                call
                for call in mock_logger.info.call_args_list
                if "Starting demo_api endpoint" in str(call)
            ]
    ...
    @pytest.mark.integration
    def test_lifespan_logs_shutdown_message(self):
    ...
    @pytest.mark.integration
    def test_main_function_logs_startup_message(self):
    ...
    def test_run_migrations_success_path(self):
        """Test run_migrations success path."""
        # Arrange
    ...
    def test_run_migrations_failure_path(self):
        """Test run_migrations failure path."""
        # Arrange
        with patch("subprocess.run") as mock_run, patch("os.chdir"), patch(
            "src.endpoints.demo_api.main.logger"
        ) as mock_logger:
    ...
    def test_run_migrations_file_not_found_path(self):
        """Test run_migrations FileNotFoundError path."""
        # Arrange
        with patch("subprocess.run") as mock_run, patch("os.chdir"), patch(
            "src.endpoints.demo_api.main.logger"
        ) as mock_logger:
    ...
    def test_run_migrations_exception_path(self):
        """Test run_migrations exception path."""
        # Arrange
        with patch("subprocess.run") as mock_run, patch("os.chdir"), patch(
            "src.endpoints.demo_api.main.logger"
        ) as mock_logger:
